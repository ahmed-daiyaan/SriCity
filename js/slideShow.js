...
const MXS_FOUR_POSITION = [
  MXS_TOP_POS,
  MXS_RIGHT_POS,
  MXS_BOTTOM_POS,
  MXS_LEFT_POS,
];
MXS_ORIGINS_POSITIONS = [
  MXS_TOP_LEFT_POS,
  MXS_TOP_RIGHT_POS,
  MXS_CENTER_POS,
  MXS_BOTTOM_LEFT_POS,
  MXS_BOTTOM_RIGHT_POS,
];
...
// Transitions
const MXS_SIMPLE_TRANSITION = "simple",
  MXS_CUTTING_TRANSITION = "cutting",
  MXS_SHAPE_TRANSITION = "shape";
const MXS_RANDOM = {
    name: "random",
    constant: false,
  },
...
function getTransition(name) {
  ...
  for (
    var i = 0;
    i < MXS_TRANSITIONS.length && MXS_TRANSITIONS[i].name != name;
    i++
  );
  if (i < MXS_TRANSITIONS.length) transition = MXS_TRANSITIONS[i];
  return transition;
}
function chooseTransition() {
  let transition =
    MXS_TRANSITIONS[Math.round(Math.random() * (MXS_TRANSITIONS.length - 1))];
  if (transition.type == MXS_SHAPE_TRANSITION) {
    transition.origin =
      MXS_ORIGINS_POSITIONS[
        Math.round(Math.random() * (MXS_ORIGINS_POSITIONS.length - 1))
      ];
  } else if (transition.name == MXS_TILES.name) {
    transition.random = Math.round(Math.random()) == 0 ? false : true;
  } else if (
    transition.name == MXS_SLICES.name ||
    transition.name == MXS_SLIDE.name
  ) {
...
(function ($) {
  $.fn.mixSlide = function (opt) {
    ...
    if (options.transition.name != MXS_RANDOM.name) {
      ...
      if (choosed_transition == null) options.transition = MXS_FADE;
      else
        options.transition = $.extend(
          true,
          choosed_transition,
          options.transition
        );
    ...
    this.each(function () {
      ...
      $obj
        .find(".mixSlide-image")
        .wrapAll('<div class="mixSlide-images"></div>');
      $obj
        .find(".mixSlide-images")
        .wrap('<div class="mixSlide-container"></div>');
      ...
      if (options.thumbs.active) {
        let thumbs_code =
          '<div class="mixSlide-thumbs ' + options.thumbs.position + '">';
        for (let i = 0; i < images.length; i++) {
          thumbs_code +=
            '<span class="mixSlide-thumb"data-image-index="' +
            i +
            '"><img src="' +
            images[i].src +
            '"/></span>';
      ...
      if (options.controls) {
          ...
          let controls_code =
            '\
						<div class="mixSlide-slide-buttons">\
							<span class="mixSlide-prev">' +
            MXS_CONTROLS_PREV_CODE +
            '</span>\
							<span class="mixSlide-next">' +
            MXS_CONTROLS_NEXT_CODE +
            '</span>\
							<span class="mixSlide-start-slide">' +
            MXS_CONTROLS_PLAY_CODE +
            '</span>\
							<span class="mixSlide-open-close-fullscreen">' +
            MXS_CONTROLS_SQUARE_CODE +
            "</span>\
						</div>";
        ...
        if (options.fullscreen) {
          $controls
            .find(".mixSlide-open-close-fullscreen")
            .html(MXS_CONTROLS_CLOSE_CODE);
        }
        if (!options.thumbs.active) {
          let points_code = '<div class="mixSlide-points">';
          for (let i = 0; i < images.length; i++) {
            points_code +=
              '<span class="mixSlide-point"data-image-index="' +
              i +
              '"></span>';
          }
          points_code += "</div>";
        ...
        $controls.find(".mixSlide-prev").click(function () {
          changeImage(MXS_BACKWARD);
        });
        $controls.find(".mixSlide-next").click(function () {
          changeImage(MXS_FORWARD);
        });
      ...
      function changeImage(dir, nextImageIndex = -1, isAutomatic = false) {
        ...
        let currentTemp = currentImageIndex,
          nextTemp = nextImageIndex;
        //Simple transition
        if (transition.type == MXS_SIMPLE_TRANSITION) {
          ...
          if (transition.name == MXS_FADE.name) {
            $imageDivs
              .eq(nextTemp)
              .css("z-index", "1")
              .fadeIn(options.animation.speed);
            $imageDivs
              .eq(currentTemp)
              .fadeOut(options.animation.speed)
              .css("z-index", "0");
          }
          //Slide Transition
            ...
            if (dir == MXS_FORWARD) {
              ...
              $imageDivs
                .eq(currentTemp)
                .animate(
                  animated_property_forward,
                  options.animation.speed,
                  function () {
                    $(this)
                      .css({ "z-index": "0" })
                      .css(animated_property_backward)
                      .hide(-1);
              ...
              $imageDivs
                .eq(nextTemp)
                .css(animated_property_forward)
                .css({ "z-index": "1" })
                .show(-1);
              $imageDivs
                .eq(nextTemp)
                .animate(
                  animated_property_backward,
                  options.animation.speed,
                  function () {
        ...
        //Cutting transition
          ...
          if (transition.name == MXS_SLICES.name) {
            ...
            for (let i = 0; i < transition.count; i++) {
              $divOver.append(
                '\
								<div class="mixSlide-div-clip slice ' +
            ...
            $divOver.find("div").each(function () {
              ...
              else clipSquare($(this), number * size, 0, size, 100);
              ...
              if (dir == MXS_FORWARD) {
                ...
                if (number % 2 == 0) {
                  $(this).animate(
                    animated_even_forward,
                    options.animation.speed
                  );
                } else {
                  $(this).animate(
                    animated_odd_forward,
                    options.animation.speed
                  );
                }
              } else {
          ...
          //Tiles transition
            ...
            if (transition.random) {
              let number = 0,
                ...
                distance_left,
                distance_top,
                elements_numbers = []; //Couples' elements (x&y)
              let element_number;
              do {
                do {
                ...
                let $element = $divOver.find(
                  'div[data-number="' + element_number + '"]'
                );
                animated_forward.left =
                  Math.floor(Math.random() * 2) == 0 ? "-100%" : "100%";
                animated_forward.top =
                  Math.floor(Math.random() * 2) == 0 ? "-100%" : "100%";
                if (dir == MXS_FORWARD) {
                  $imageDivs.eq(nextTemp).css("z-index", "1").show(-1);
                  $imageDivs.eq(currentTemp).hide(-1).css("z-index", "0");
        ...
        //Shape transition
          ...
          if (transition.origin != MXS_CENTER_POS)
            if (transition.name == MXS_CIRCLE.name) radius = 150;
            else radius = 100;
          switch (transition.origin) {
            case MXS_TOP_LEFT_POS:
              (x = 0), (y = 0);
              break;
            case MXS_TOP_RIGHT_POS:
              (x = 100), (y = 0);
              break;
            case MXS_BOTTOM_LEFT_POS:
              (x = 0), (y = 100);
              break;
            case MXS_BOTTOM_RIGHT_POS:
              (x = 100), (y = 100);
              break;
          }
          origin = x + "% " + y + "%";
          ...
          if (transition.name == MXS_CIRCLE.name) {
            if (dir == MXS_FORWARD) {
                ...
                { step: radius },
                {
                  ...
                  step: function (val) {
                    $imageDivs
                      .eq(nextTemp)
                      .css({
                        "clip-path": "circle(" + val + "% at " + origin + ")",
                      });
                  },
                  always: function () {
                    $imageDivs.eq(currentTemp).hide(-1);
              ...
              $imageDivs
                .eq(currentTemp)
                .css("clip-path", "circle(" + radius + "% at " + origin + ")");
      ...
      if (options.autoplay) launchAnimation();
    });
    return this;
  };
